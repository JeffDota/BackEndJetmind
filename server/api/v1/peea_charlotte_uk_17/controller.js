...
enviarPdfCH17 = async (doc) => {
  ...
  setTimeout(async () => {
    ...
    const pdfDefinition = {
      ...
      content: [
        ...
        {
          text: [
            { text: 'Mencione con quién vive el/la estudiante: ' },
            `${doc.pregunta7}`,
          ]
        },
        '\n\n',
        ...
        '\n\n',
        {
          table: {
            body: [
              ...
              ['¿Su hijo/a tiene alguna dificultad visual?', `${doc.pregunta8.respuesta}`, `${doc.pregunta8.observacion}`],
              ['¿Su hijo/a tiene alguna dificultad auditiva?', `${doc.pregunta9.respuesta}`, `${doc.pregunta9.observacion}`],
              ['¿Su hijo/a tiene alguna dificultad de aprendizaje?', `${doc.pregunta10.respuesta}`, `${doc.pregunta10.observacion}`],
        ...
        '\n\n',
        {
          table: {
            body: [
              ['PREGUNTA', 'RESPUESTA', 'OBSERVACIONES'],
              ['¿Habla inglés en su casa? ', `${doc.pregunta11.respuesta}`, `${doc.pregunta11.observacion}`],
              ['¿Ha tomado otro curso de inglés?', `${doc.pregunta12}`, ``],
              ['¿Ha vivido en algún país de habla inglesa?', `${doc.pregunta13}`, ``],
              ['¿Asiste a escuela / colegio bilingüe?', `${doc.pregunta14}`, ``],
              ['¿Encuentra fácil hablar cualquier idioma en público?', `${doc.pregunta15}`, ``],
        ...
        {
          text: [
            { text: 'Explique el motivo principal por el cual decidió tomar el entrenamiento y cuales son las expectativas para su representado: ' },
            `${doc.pregunta16}`,
          ]
        },
        '\n',
    ...
    pdfDoc.pipe(fs.createWriteStream(`PEEA-CH-17-${contrato.codigo}-${estudiante.nombresApellidos}.pdf`));
    pdfDoc.end();
  }, 2500);
  ...
  setTimeout(async () => {
    const esperar = await envioEmail.transporter.sendMail({
      ...
      subject: `PEEA CHARLOTTE 17|  ${contrato.codigo} | ${estudiante.nombresApellidos}`,
      ...
      attachments: [
        {
          filename: `PEEA-CH-17-${contrato.codigo}-${estudiante.nombresApellidos}.pdf`,
          path: path.join(__dirname, `../../../../PEEA-CH-17-${contrato.codigo}-${estudiante.nombresApellidos}.pdf`),
